---
title: LF File Manager Configuration Written in Lumesh
...
LF File Manager Configuration Written in Lumesh

Shebang is only for syntax highlighting

ifs settings, needs to be used with `LUME_IFS_MODES` setting. Better solution is not to directly use `$fx` containing multiple variables as command arguments, but to split it using `$fx.lines()` and use it as arguments.

shellopts `-s` means strict mode. Non-strict mode allows using literals directly as variables, while strict mode can achieve faster parsing speed.

shell specifies lumesh or lume as script parser.

[Syntax demonstration for writing LF configuration files](/zh-cn/cases/case_lf)
...
map rz &zed '.'
cmd cmus-play &{{
    # sock=/run/user/1000/cmus-socket default socket path, no need to specify
