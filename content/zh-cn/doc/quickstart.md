...
> [!TIP] **从0.16.0开始，CFM模式和普通模式的差别进一步缩小**
> 普通模式能够识别更多的命令上下文，`chmod +x` `ping 1.1.1.1`等命令也可以直接在普通模式下使用了。
>
> 命令模式下也可使用数字和相关运算了，但运算符两侧应空格。
>
> 现在你只需要注意两个区别：
> - 单个单词：CFM模式下为命令；普通模式下为符号
> - 命令模式下，`-` `+` `.` `=` 这几个符号通常被视为普通字符串，除非两侧空格
