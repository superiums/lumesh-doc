...
**`ddebug <args>...`** - 打印参数的紧凑调试表示形式（类似 Rust 的 `{:?}` 格式）
- **参数**：
  - `args` (可变): `Any...` - 要调试打印的值
- **用途**：显示数据的内部结构，便于调试
...
**`format <template> <text>`** 使用变量格式化字符串（支持位置参数和命名参数）
例如：
```bash
let a,b,c = 10
format 'a={a} b={b} c={}' c
...
**`symof <value>`** - 获取数据字面量类型
- **参数**：
  - `value` (必需): `Any` - 要检查类型的值
- **返回**：`String` - 类型名称
- **示例**：`typeof 123` 返回 "Integer"
...
**注意**：如果您正在CFM（命令优先）模式，您应使用`lume -m`或在行首添加`:`，才能获得正确的结果。
...
**`exec_str <string>`** - 在新的环境中求值字符串
- **参数**：
  - `string` (必需): `String` - 包含代码的字符串
- **用途**：执行字符串形式的代码

**`repeat <expr> <count>`** - 重复执行表达式指定次数
- **参数**：
