...
"abc" | .upper()          # 管道方法式（省略库名）
```
由于函数式参数的优先级低于命令式，函数式支持**lamda**、**管道**、**block块**等类型的参数；命令式中只能加括号包装为组。
...
默认情况下，管道 会把左侧结果 注入到右侧表达式的 **第一个参数位置** ， 如需自定义，可在管道右侧明确使用`_`指定参数注入位置。
...
### 2. 传 `<fn>` 参数时，可使用箭头函数
如果函数是字面量，必须使用函数式参数，不能使用命令式参数。

`filter`/`map`/`find`/`fold`/`sort` 等 hint 里写的 `<fn>` 都是这种语法：
...
# 命令式无法成功解析
list.filter data  x -> x > 0         # 错误
list.filter data  (x -> x > 0)       # 可以，但不推荐
```
...
任何语句（内置函数/运算操作/系统命令/自定义函数等）出错时，可以配合错误处理操作符控制行为，而不必用 `try/catch`：

```bash
fs.read("no_such_file") ?: "default content"   # 出错则用默认值
fs.read("no_such_file") ?: handler             # 出错则调用错误处理，相当于catch
fs.read("no_such_file") ?.                     # 忽略错误，继续执行
risky_call() ?!                                # 出错立即终止管道（管道之外会自动终止，无需显式终止）
```

### 8. lume中`&&`和`||`是纯逻辑运算符，这和bash不同
如果需要达到同样的效果，请使用：
```bash
cmd1 &: cmd2 ?: cmd3
```
注意含义和bash中一样：当cmd1成功时执行cmd2, 前两个命令中任意一个失败都会执行cmd3
...
> 这里精选一些模块函数加以说明，作为 `help` 信息的辅助，让你快速掌握 lume 内置函数用法。
...
这两个函数都是为了兼容传统编写习惯，更严谨的做法是使用 `use` 语句导入模块。区别是：

- `use` 使用独立的命名空间
- `use` 内的代码不会立即执行，直到显式调用某个函数
- `use` 导入的命名空间仅有 `fn` 和模块依赖的 `use`

#### `dig`/`get`（top/list/map 共用）：点号路径可以混合访问 Map key 和 List 下标
...
| 部分 | 含义 | 说明 |
|---|---|---|
| `name` (可选) | 变量名 | `:`之前，省略则从位置参数中取 |
| `*` | 填充符号 | 可以为任意char，省略则为 `' '` |
| `>` / `^` / `<` | 对齐方式 | 分别为右对齐/居中对齐/左对齐 |
| `10` | 对齐宽度 | 可以是任意整数 |

**format**的功能和**`` `template` ``模板字符串功能有部分重叠，都可以插值。

但format重在格式化输出，template重在插值，如果没有格式化需求，推荐使用template，性能更好。

#### `assert` / `when`：失败终止 vs 条件才执行
...
**`pad_start`/`pad_end`/`center`**：第三个可选参数只取其首字符作为填充符，传多字符字符串也只生效一个字符；若原字符串长度已达标，直接原样返回、不截断。

**`color`/`color_bg`**：颜色值支持三种写法——`#hex`、颜色名（查 `string.colors()`）、或 `r,g,b` 三段式，不支持 `rgb(...)` 这种带函数名的写法。
**`clr`/`clr_bg`**：颜色值0..=255。

#### `string.slice`（`s[a:b]` 语法）越界行为一致：静默截断不报错
...
#### `string.remove_prefix`/`remove_suffix`：找不到就原样返回，不是报错

```bash
"hello.txt".strip_suffix(".txt")   # -> "hello"
"hello.txt".strip_suffix(".md")    # 后缀不匹配，原样返回 "hello.txt"（不报错）
```
...
`sort` 支持三种第二参数形态：`fn(a,b)->int|bool`、单个字段名字符串、多个字段名（列表或多参数），按类型分派。`key_fn` 返回 `Integer`（比较结果 -1/0/1）或 `Boolean`（`true` 表示 a>b），两种返回类型都认。其中 `+`/`-` 可作为字段前缀表示升序或降序。`list`/`table`/`string` 中的 `sort` 内在实现统一，但 string元素 中没有字段名，list元素 中可以用列索引。
...
key 既可以是 `fn(item)->key` 也可以是字段名字符串，结果统一转成字符串作为分组键。
...
#### `list.find` / `bset.find` / `map.find` 返回**元素本身**，`list.position` / `table.position`/`rposition` 返回**索引**

`find` 系列（bset/map 模块）是"给条件，拿命中的那条数据"；`table.position`/`rposition` 是"给条件，拿命中的行号"。`list.find` vs `list.find_last`：都能传值或函数，且都能指定起始下标。
...
#### `list.remove` vs `list.remove_at`：按值删 vs 按索引删
...
`remove` 找不到匹配值时**静默返回原列表**，不报错；`remove_at` 索引越界同样静默返回原列表。
...
#### `where` / `table.filter`：条件里可直接用列名和 `NR`/`NF`

`where` 的第二个参数是**惰性表达式**，执行时会把当前行拆成局部变量再求值：行按 key 展开成同名变量，并统一注入 `NR`（行号）。

```bash
fs.ls -lh | where(size > 5K)      # size 是列名，直接当变量用
fs.ls -l | where(NR > 0)          # 跳过第一行数据（表头行不受影响）
```

**不要**把 `where` 的条件写成字符串或函数，它是直接内联表达式，变量名来自数据本身。如想使用函数，可使用 `table.filter` 函数，下面两种方式可得到相同结果：
...
#### `select`/`get_column`（table 模块）与顶层 `select`：只对 `Table` 行生效

`select` 用于选择多列，返回 table；`get_column` 用于选择单列，返回 list。
...
#### `table.from_maps`：表头顺序取决于首次出现顺序，而非字母序
...
fs.ls -l -a /tmp        # 等价于
fs.ls -la /tmp
fs.ls /tmp -l -a        # 可以，但不推荐，尤其路径为`/`时，会被误认为是除法
```

`-l`（默认：详细信息）会额外带出 `type`/`size`/`modified` 字段，`-t` 控制 `modified` 是时间戳还是 `DateTime`，`-u`/`-m` 分别独立追加 `user`/`mode` 字段（不依赖 `-l`），`-p` 追加完整 `path` 字段——这些标志相互独立叠加，不是互斥的："先选类别再选字段"这种理解是错的，每个标志只负责往结果 map 里加一个字段。
...
**`jq` 的 `select(...)` 支持数字比较**（形如 `.field>数字`），支持逻辑组合（`and`/`or`）、字符串比较或嵌套 `select`：

```bash
from.jq('{"list":[{"n":1},{"n":5}]}', '.list|select(.n>2)')    # OK
from.jq(data, '.list|select(.n>2 and .n<10)')                  # 逻辑组合
```
...
- **`from.script` 只解析成表达式（AST），不自动执行。

---

### `reg`（正则）模块

#### `is_match` 判断的是整个正则能否在文本中命中（非锚定整串相等）；想判断"整串完全等于"需要自己在 pattern 前后加 `^`/`$`。

```bash
reg.is_match("2025-01-01", g'\d{4}-\d{2}-\d{2}')     # 只判断整体是否匹配 -> true/false
```

#### `reg.find` / `reg.find_all`：单个匹配 vs 全部匹配
...
#### `reg.capture` / `reg.captures` / `reg.named_captures`：分组的三种取法
...
用位置分组还是命名分组取决于正则里有没有写 `(?<name>...)`，三者不是互相替代关系。`named_captures` 的模式必须用 `(?<name>...)` 具名捕获组语法，只有命名捕获组会出现在结果 Map 里，普通 `(...)` 捕获组会被忽略：
...
**`gt`/`ge`/`lt`/`le`/`eq`/`ne` 与运算符 `>`/`<`/`==` 完全等价**，只是函数形式，方便在管道里当作参数传递：
...
内部采用"先累加整数、遇到浮点再切换"的优化逻辑，纯整数输入才会得到整数结果：
...
#### `rand.choose` / `rand.sample`：取一个 vs 不重复取多个
...
- **`raw_mode`、`alt_screen` 是手动开关，不会自动恢复**：进入 raw mode 或备用屏幕后，如果脚本中途报错退出，终端会卡在该模式下（不像 `ui.pick` 内部会话结束自动清理），务必确保成对调用或配合 `?:` 兜底 。
- **`cursor_up`/`down`/`left`/`right`参数含义都是"移动距离"而非"目标坐标"** ，和 `cursor_to(x,y)` 的绝对定位语义不同，别混用 。
...
`read_key` 内部会自动临时开关 raw mode（无需你手动调用 `raw_mode`），一次性读取单个按键后自动恢复；而 `read_line`/`read_password` 走的是标准输入行读取，不需要 raw mode 。
...
配置 Map 里能传 `page_size`、`starting_cursor` 等字段，字段名即 `inquire` 库的配置项：
...
| `sorter` | `fn(integer,integer) -> Integer`（正数为 Greater，负数为 Lesser） |
...
| `keep_filter` | Boolean |
| `validator` | `fn(selected_items) -> Boolean/String`（字符串表示无效提示） |
