...
| pick       | 单选（选择一个选项）    | `<选项列表/字符串> [提示或配置]`        | 任意类型      |
| multi_pick | 多选（选择多个选项）    | `<选项列表/字符串> [提示或配置]`        | List          |
...
1. 简单单选：
```bash
let color = (ui.pick  ["红", "绿", "蓝"] "选择颜色：")
```

2. 使用字符串选项源：
```bash
let fruits = "苹果\n香蕉\n橙子"
let choice = (ui.pick fruits "选择喜欢的水果：" )
```

3. 使用详细配置：
```bash
let oss = ["win","linux","mac"]
let choice = ui.pick(oss, {
    msg: "请选择操作系统：",
    page_size: 5,
    starting_cursor: 1,
    })
