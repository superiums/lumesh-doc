...
**`words_quoted <string>`** - 按空白字符分割为单词列表,引号内空白除外
- **参数**：`string` (必需): `string` - 要分割的字符串
- **返回**：`List[string]` - 单词列表
...
```bash
# 处理文本数据的管道操作
"Hello World" | string.upper()() | string.replace "WORLD" "UNIVERSE"
# 结果: "HELLO UNIVERSE"

# 分析文本内容
"The quick brown fox" | string.words() | list.map(string.upper())
